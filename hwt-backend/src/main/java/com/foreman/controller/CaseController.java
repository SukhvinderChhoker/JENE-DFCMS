...
public class CaseController {
    ...
    public ResponseEntity<CaseDTO> createCase(@RequestBody CaseDTO caseDTO, HttpServletRequest request) {
        ...
        return ResponseEntity.ok(caseService.createCase(caseDTO, user.getId()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<CaseDTO> updateCase(@PathVariable Long id, @RequestBody CaseDTO caseDTO, HttpServletRequest request) {
        User user = requireAuth(request);
        if (!roleHelper.canManageCases(user)) {
            throw new RuntimeException("Access denied: Cannot update cases");
        }
        return ResponseEntity.ok(caseService.updateCase(id, caseDTO, user.getId()));
    }
    ...
    public ResponseEntity<CaseDTO> changeStatus(
        ...
        if (!roleHelper.canManageCases(user)) {
            throw new RuntimeException("Access denied: Only Case Managers and Admins can change case status");
        }
        String status = request.get("status");
        String reason = request.get("reason");
        return ResponseEntity.ok(caseService.changeStatus(id, status, reason, user.getId()));
    }
    ...
    public ResponseEntity<CaseDTO> closeCase(
        ...
        User user = requireAuth(httpRequest);
        if (!roleHelper.canManageCases(user)) {
            throw new RuntimeException("Access denied: Only Case Managers and Admins can close cases");
        }
        String reason = request.get("reason");
        return ResponseEntity.ok(caseService.closeCase(id, reason, user.getId()));
    }
    ...
    public ResponseEntity<CaseDTO> authorizeCase(
        ...
        return ResponseEntity.ok(caseService.authorizeCase(id, authorize, reason, user.getId()));
    }
    ...
    public ResponseEntity<TaskDTO> createTaskForCase(
        ...
        return ResponseEntity.ok(taskService.createTask(id, taskDTO, user.getId()));
    }
    ...
    public ResponseEntity<EvidenceDTO> createEvidenceForCase(
        ...
        return ResponseEntity.ok(evidenceService.createEvidence(id, evidenceDTO, user.getId()));
    }
    ...
    public ResponseEntity<Map<String, String>> linkCases(
        ...
        String reason = (String) request.get("reason");
        caseService.linkCases(id, linkedCaseId, reason, user.getId());
    ...
    public ResponseEntity<Map<String, String>> unlinkCases(
        ...
        caseService.unlinkCases(id, linkedId, reason, user.getId());
